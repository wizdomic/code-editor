...
async function executeViaProxy(
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/api/execute`, {
      ...
      body: JSON.stringify({ script: code, language, versionIndex, stdin }),
    });
...
const languageConfigs: Record<string, LanguageConfig> = {
  javascript: {
    execute: async (code: string, stdin = ''): Promise<ExecutionResult> => {
      try {
        const output: string[] = [];
        const context = {
          console: {
  ...
  typescript: {
    execute: async (code: string, stdin = ''): Promise<ExecutionResult> => {
      ...
      return languageConfigs.javascript.execute(stripped, stdin);
    },
  },
  ...
  ...Object.fromEntries(
    Object.entries(JDOODLE_LANGUAGES).map(([key, { language, versionIndex }]) => [
      key,
      {
        execute: (code: string, stdin = '') => executeViaProxy(language, versionIndex, code, stdin),
      } as LanguageConfig,
    ])
  ),
};
...
export const executeCode = async (
  ...
  return await config.execute(code, stdin);
};  